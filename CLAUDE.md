...
python -m plaiground_host.api_server             # 서버, http://127.0.0.1:8770
```
...
- **`var/generated/`의 스크립트를 덮어쓰지 않는다.** 사용자가 직접 고친 학습 스크립트가 들어 있다. `generate()`를 기본 출력 폴더로 호출하는 테스트나 스크립트를 만들지 않는다. 점검에는 임시 폴더를 쓴다.
...
- **`origin` 원격에 푸시하지 않는다.** 구버전 저장소다. 최신 원격은 `pg_demo`이다.
...
- 폴더 위치를 코드에 하드코딩하지 않는다. 호스트는 `plaiground_host/paths.py`, SDK는 `plaiground_telemetry/paths.py`에서 가져다 쓴다. `Path(__file__).parent.parent...`로 저장소 구조를 거슬러 올라가지 않는다.
- 실행 중 생기는 파일은 `var/` 아래에만 쓴다.
