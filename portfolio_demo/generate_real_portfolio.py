...
실제 학습 텔레메트리로 포트폴리오를 생성한다.
  ...
  python -m portfolio_demo.generate_real_portfolio              # 최신 raw_telemetry.json
  python -m portfolio_demo.generate_real_portfolio --run <id>   # .telemetry/runs/<id>.json

run_id를 주면 산출물도 portfolio_demo/output/<id>.{html,json}으로 따로 보관된다.
"""
import json
import sys
...
def build_real_portfolio(run_id: str | None = None) -> None:
    base = Path(__file__).parent / ".telemetry"
